...
<script async
	src="https://www.googletagmanager.com/gtag/js?id=UA-132577341-1"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'UA-132577341-1');
</script>
<script type="text/javascript" src="//wcs.naver.net/wcslog.js"></script>
<script type="text/javascript">
if(!wcs_add) var wcs_add = {};
wcs_add["wa"] = "d76da48fb81240";
wcs_do();
</script>
<script type="text/javascript" src="//wcs.naver.net/wcslog.js"></script>
<script type="text/javascript">
if(!wcs_add) var wcs_add = {};
wcs_add["wa"] = "d72903ce2d5c08";
wcs_do();
</script>
<!-- Global site tag (gtag.js) - Google Analytics -->
<script async
	src="https://www.googletagmanager.com/gtag/js?id=UA-132537133-1"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'UA-132537133-1');
</script>
<meta charset="UTF-8">
...
<!-- Global site tag (gtag.js) - Google Analytics -->
<script async
	src="https://www.googletagmanager.com/gtag/js?id=UA-132460737-1"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());
...
<body>
	...
	<%
		...
		// 		if(id==null){
		// 			list=(ArrayList<AddCartDTO>)session.getAttribute("list");
		// 		}
		...
		<div class="container-menu-header">
			<div class="topbar">
				<div class="topbar-social">
					<a href="http://pf.kakao.com/_BYHxaj/chat"> <img
  ...
  //]]>
 	...
 	if (admin != null) {
 %> <%=admin%>
						님 안녕하세요 &nbsp;&nbsp; <a href="_logout.jsp" onclick="signOut()">Logout</a>
						<%
							} else {
						%> <%
 	if (id != null) {
 %> <%=name%> 님 안녕하세요 &nbsp;&nbsp; <a
						href="_logout.jsp" onclick="signOut()">Logout</a> <%
 	} else {
 %> <span
						class="topbar-email"> <a href="login.jsp" id=login_main>로그인</a>
				...
				<div class="wrap_menu">
					<nav class="menu">
						<ul class="main_menu">
							...
							<li id="product" class="menu-li"><a
								href="product.jsp?kind=all">Shop</a></li>

							<li id="blog" class="menu-li"><a href="notice.jsp">Board</a>
							</li>

							<li id="about" class="menu-li"><a href="about.jsp">About</a>
							</li>

							<li id="contact" class="menu-li"><a href="contact.jsp">Contact</a>
							</li>
						</ul>
					</nav>
				</div>
 ...
 --%>
					...
					<a href="member_mod.jsp" class="header-wrapicon1 dis-block"> <img
					...
					<div class="header-wrapicon2">
						<img src="images/icons/icon-header-02.png"
							...
							class="header-icons-noti"><%=list.size()%></span>
						<script type="text/javascript">
						...
						<div class="header-cart header-dropdown">
							<ul class="header-cart-wrapitem">
								...
								<input type="hidden" id="pid<%=i%>"
									value="<%=list.get(i).getpId()%>">
								<input type="hidden" id="size<%=i%>"
									value="<%=list.get(i).getSize()%>">

								<li class="header-cart-item" id="cart<%=i%>">
									<div onclick="deleteAddCart(<%=i%>)"
									...
									<div class="header-cart-item-txt">
										<a href="product-detail.jsp?pId=<%=list.get(i).getpId()%>"
											class="header-cart-item-name"> <%=pDto.getName()%>
										</a> <span class="header-cart-item-info"> <%=list.get(i).getCount()%>개
											(<%=list.get(i).getSize()%>) X <%=pDto.getPrice()%>원
										</span>
									</div>
								</li>
								<%
							...
							<div class="header-cart-total">
								<font size="1">장바구니로 이동 시 업데이트 됩니다.</font><br> Total:
								<%=sum%>원
							</div>
		...
		<div class="wrap-side-menu">
			<nav class="side-menu">
				<ul class="main-menu">
					...
					<li class="item-menu-mobile"><a href="product.jsp?kind=all">Shop</a>
					</li>

					<li class="item-menu-mobile"><a href="product.jsp?kind=all">Sale</a>
					</li>
