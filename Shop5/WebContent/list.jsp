			...
			<div class="row">
				<div class="col-md-8 col-lg-9 p-b-75">
					...
					<table class="table table-striped" style="text-align: center; 1px solid #dddddd">
						...
						<tbody>
							<%
								...
								ArrayList<BbsDTO> list = dao.searchList(bbsCategories, col, word);
								for(int i=0; i<list.size(); i++) {
							%>
								<tr>
									<td id="bbsId"><%=list.get(i).getBbsId() %></td>
									<td><a href="read.jsp?bbsId=<%=list.get(i).getBbsId() %>&bbsCategories=<%=list.get(i).getBbsCategories() %>"><%=list.get(i).getBbsTitle() %></a></td>
					...
					<div class="pagination flex-m flex-w p-r-50">
						<%	
							if(dao.getNext("notice") == 1) {
								pageNumber = 1;
							}
							else {						
								pageNumber = (dao.getNext("notice") - 2) / 10 + 1;
							}
							int cnt = 1;
							for(int i=1; i<=pageNumber; i++) {
								if(i == cnt) {
						%>
								<a href="notice.jsp?pageNumber=<%=i %>" class="item-pagination flex-c-m trans-0-4 active-pagination"><%=i %></a>
						<%
								}
								else {
						%>
									<a href="notice.jsp?pageNumber=<%=i %>" class="item-pagination flex-c-m trans-0-4"><%=i %></a>
						<%
								}
							}
						%>
